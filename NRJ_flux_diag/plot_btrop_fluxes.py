...
adapted from plot_fluxes_lf.py
NJAL Apr 2018 '''
...
from matplotlib_perso import MidpointNormalize as midnorm
...
parser.add_argument("--field",help="field to plot (bcFlux or divFluxBC)",default="cterm")
...
ncbt = MFDataset(fil_flux,aggdim='eta_rho')
if args.imax is None:
    ifin = ncbt.dimensions['time'].size
else:
    ifin = args.imax
...
# initialize: use AVG
if whatoplot == "cterm":
    toplot = ncbt.variables['Ct_avg'][:]
    if lascale == "linear":
        clims = np.array([-2, 4])*np.nanstd(toplot)
        toplot = toplot.clip(*clims) # need this with current vesion of MidpointNormalize
    elif lascale == "log":
        clims = np.array([-1,1])*4*np.nanstd(toplot)
    bextend = "both"
    try:
        units = r"${}$".format(ncbt.variables['Ct_avg'].units)
    except:
        units = r"$W/m^2$"
...
for it in range(ideb+ires,ifin,ist):
    #Fx = np.ma.masked_invalid(ncbt.variables['puint'+whatokind][:,:,it])
    #Fy = np.ma.masked_invalid(ncbt.variables['pvint'+whatokind][:,:,it])
    #Fnorm = np.sqrt(Fx**2 +Fy**2)
    if whatoplot == "cterm":
        toplot = np.ma.masked_invalid(ncbt.variables['Ct'+whatokind][:,:,it])
    #elif whatoplot == "divFluxBC":
        ##toplot = np.gradient(Fx,itg.cumtrapz(1./pm, axis=1 , initial=0), axis=1) \
                 ##+ np.gradient(Fy,itg.cumtrapz(1./pn, axis=0 , initial=0), axis=0)
        #toplot = np.pad(diff_2d_ongrid(Fx, pm, axis=1), ((0,0),(1,1)), "constant")*1e3 \
...
##### update plot
    ...
    if doverb:
        print('it {0}, timing :'.format(it),time.clock()-tmes,time.time()-tmeb)
        tmes, tmeb = time.clock(), time.time()

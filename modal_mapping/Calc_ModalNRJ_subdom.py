...
# WARNING some of the lines (at the beginning, parameters and paths) are not used -- inheritance from previous versions and lazyness in cleaning
# 
...
data_mod = store+simdir+'data/modemap/{}_modemap-b.nc'.format(simul)
data_nrj = store+simdir+'data/modemap/{}_modenrj-b.nc'.format(simul)

grid_file = store+"/lucky_corgrd.nc"
doms_file = work+simdir+"data/subdomains_lucky.json"
dirpic = work+simdir+'pictures/modal_mapping/'
dosavefig = True 
dostore = True  # store results in a netcdf file and in a csv file

filout = scratch+simdir+'data/modemap/{}_mode_scatdiag_lg-b.nc'.format(simul)
doappend = False
csvout = work+simdir+"data/{}_NRJ_diags_lg-b.csv".format(simul)
...
indt, = np.where((times>=720)&(times<=1080)) #np.arange(900, 1020)
...
for imod in indm:
    ...
    for inod in indm[imod+1:]:
        ...
        prav = phim*phin / (lam-lan) * (                            pmod[indt,imod,...]*lam*(umod[indt,inod,...]*dhdx + vmod[indt,inod,...]*dhdy) +                             pmod[indt,inod,...]*lan*(umod[indt,imod,...]*dhdx + vmod[indt,imod,...]*dhdy) )

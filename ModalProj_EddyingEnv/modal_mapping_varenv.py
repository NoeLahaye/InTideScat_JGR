...
filname = 'modemap'     # suffix to create netCDF file name (modal amplitudes)
filmode = "v-modes"     # for mode structure
bastate_filename = fildir+simulname+"_subsamp_stratif.nc"
npy, npx = (8,8)        # number of chunks in y, x (mpi)
ideb = 2705             # it start computing modal amplitude
...
if nbproc==1:
    ...
    ipy, ipx = 0, 0                     # mainly for debugging or testing
else:
    if npx*npy != nbproc:
...
if npx*npy == 1:
    ...
    ncfilemode = fildir+simulname+'_{0}.{1}_{2}.nc'.format(filmode,ipy,ipx)
...
if os.path.exists(ncfilemode) and doappend:
    ...
    ncw.createDimension('xi_rho',nbx)
    ncw.createDimension('eta_rho',nby)
    # attributes
    ...
    ncwar = ncw.createVariable('mode_speed','f',('time','mode','eta_rho','xi_rho'))
...
wmodeb = np.full((2,nx,ny,Nz+1,Nmodes),np.nan,dtype=np.float32)
pmodeb = np.full((2,nx,ny,Nz,Nmodes),np.nan,dtype=np.float32)
norm_w = np.full((2,nx,ny,Nmodes),np.nan,dtype=np.float32)
norm_p = np.full((2,nx,ny,Nmodes),np.nan,dtype=np.float32)
Nsqb = np.full((2,nx,ny,Nz+1),np.nan,dtype=np.float32)
bbase = np.full((2,nx,ny,Nz),np.nan,dtype=np.float32)
...
(wmodeb[1,...], pmodeb[1,...]), (cmap, norm_w[1,...], norm_p[1,...]) \
        , (Nsqb[1,...], bbase[1,...]) = get_modes_interp_file( \
                        bastate_filename, it0, [i1,i2,j1,j2], Nmodes \
...
for it in range(ideb+ireb,ifin):
    ...
    res = ( np.nansum((data*zw)[...,None]*wmod,axis=-2) \
            + grav*(data[...,-1,None]/bvf[...,-1,None])*wmod[...,-1,:] ) / wnorm
    ncw = Dataset(ncfilename,'a')
    ncw.variables['b_amp'][it,...] = res.T
    data = get_redpres(data,zr,0) + grav*zeta[...,None]
    res = np.nansum((data*zw)[...,None]*pmod,axis=-2) / pnorm
    ncw.variables['p_amp'][it,...] = res.T
    data = var('w',simul,coord=coords).data[::nst,::nst,:]
    res = ( np.nansum((data*Nsqr*zw)[...,None]*wmod,axis=-2) \
            + grav*data[...,-1,None]*wmod[...,-1,:] ) / wnorm
    ncw.variables['w_amp'][it,...] = res.T
    data = tools.u2rho(var('u',simul,coord=coordu).data)[1:-1:nst,::nst,:]
    res = np.nansum((data*zw)[...,None]*pmod,axis=-2) / pnorm
    ncw.variables['u_amp'][it,...] = res.T
    data = tools.v2rho(var('v',simul,coord=coordv).data)[::nst,1:-1:nst,:]
    res = np.nansum((data*zw)[...,None]*pmod,axis=-2) / pnorm
    ncw.variables['v_amp'][it,...] = res.T
    ncw.variables['time'][it] = simul.time
    ncw.variables['ocean_time'][it] = simul.oceantime/3600.
    ncw.close()
    if doverb:
